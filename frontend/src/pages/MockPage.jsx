  ...
  return (
    <>
      <header className="mb-9 flex items-end justify-between gap-6 max-[820px]:flex-col max-[820px]:items-start">
        <div><p className="mb-2.5 text-[11px] font-bold tracking-widest uppercase text-muted">{page.eyebrow}</p><h1 className="font-display text-[clamp(28px,3vw,42px)] leading-[1.08] tracking-tighter text-ink">{page.title}</h1><p className="mt-3 text-[13px] leading-[1.55] text-muted">{page.description}</p></div>
        {isManagedPage ? (
      ...
      <section className="min-h-90 rounded-[14px] border border-line bg-white p-5.5 max-[560px]:p-4">
        <div className="mb-5 flex items-start justify-between gap-4.5"><div><h2 className="font-display text-[17px] tracking-[-0.03em]">{page.heading}</h2><p className="mt-1.5 text-[13px] leading-[1.55] text-muted">Mock data for the first frontend pass</p></div><span className="rounded-[20px] bg-blue-soft px-2.25 py-1.5 text-[11px] font-bold text-blue">{items.length} items</span></div>
        {items.length === 0 ? (
          <div className="grid justify-items-center px-6 pt-14.5 pb-11.5 text-center">
            <span className="mb-4 grid size-12 place-items-center rounded-[14px] bg-blue-soft text-blue"><Icon name={icon} size={22} /></span>
            <h3 className="font-display text-[17px]">{emptyCopy[type][0]}</h3>
            <p className="mb-5 mt-2 max-w-90 text-[13px] leading-normal text-muted">{emptyCopy[type][1]}</p>
          ...
          <div className="grid">
            {items.map(([label, detail, action], index) => (
              <div className="flex items-center gap-3 border-t border-line py-4" key={`${label}-${index}`}>
                <span className="grid size-8.5 shrink-0 place-items-center rounded-lg bg-blue-soft text-blue"><Icon name={icon} size={16} /></span>
                <span className="grid flex-1 gap-1.25"><strong>{label}</strong><small className="text-xs text-muted">{detail}</small></span>
                {isManagedPage ? (
                  <span className="flex items-center gap-2">
                    <span className="rounded-[20px] bg-brown-soft px-2.25 py-1.25 text-[10px] font-bold text-brown">{action}</span>
                    {type === 'receipts' && <button className="rounded-md border border-line bg-transparent px-1.75 py-1.25 text-[10px] font-bold text-blue hover:border-blue hover:bg-blue-soft" onClick={() => setSelectedItem(index)}>View</button>}
                    {type !== 'insights' && <button className="rounded-md border border-line bg-transparent px-1.75 py-1.25 text-[10px] font-bold text-blue hover:border-blue hover:bg-blue-soft" onClick={() => editItem(index)}>Edit</button>}
                    <button className="rounded-md border border-line bg-transparent px-1.75 py-1.25 text-[10px] font-bold text-brown hover:border-brown hover:bg-brown-soft" onClick={() => deleteItem(index)}>Delete</button>
                  </span>
                ) : <span className="rounded-[20px] bg-brown-soft px-2.25 py-1.25 text-[10px] font-bold text-brown">{action}</span>}
                {type === 'receipts' && selectedItem === index && <div className="mt-1 ml-11.5 grid basis-full gap-1 rounded-lg bg-paper p-3 text-xs text-muted"><strong className="text-ink">{label}</strong><span>{detail}</span><small className="text-[11px]">Mock receipt detail view · no backend connected</small></div>}
              </div>
            ))}
          </div>
        )}
