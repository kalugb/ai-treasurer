	...
	return (
		<>
			...
			<section className="grid content-start gap-4.5 rounded-[14px] border border-line bg-white p-5.5 max-[560px]:p-4">
				<div className="mb-5 flex items-start justify-between gap-4.5"><div><h2 className="font-display text-[17px] tracking-[-0.03em]">{editing === null ? 'Add an organization' : 'Edit organization'}</h2><p className="mt-1.5 text-[13px] leading-[1.55] text-muted">Local-only for now. Changes are kept in this page while it is open.</p></div><span className="rounded-[20px] bg-blue-soft px-2.25 py-1.5 text-[11px] font-bold text-blue">{organizations.length} {organizations.length === 1 ? 'organization' : 'organizations'}</span></div>
				<form className="flex items-end gap-3 pb-5.5 max-[820px]:flex-col max-[820px]:items-stretch" onSubmit={save}>
					<label htmlFor="organization-name" className="grid flex-1 gap-1.75 text-xs font-bold text-ink">Organization name<input id="organization-name" className="h-10.5 w-full rounded-[7px] border border-line bg-paper px-3 text-ink outline-none focus:border-blue focus:shadow-[0_0_0_3px_var(--color-blue-ring)]" value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Acme Inc." /></label>
